...
//==============================================================================
Guitar_ampAudioProcessor::Guitar_ampAudioProcessor(): bass_filter(), mid_filter(), treble_filter(), cabinet(), tube()
...
void Guitar_ampAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float signal;
    float signal_bass, signal_mid, signal_treble;
    ...
    for (auto sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            /* ########## */
            signal *= masterVolume;
            outBuffer[sample] = signal;

        }
        if (totalNumOutputChannels == 2)
        {
            auto* outBuffer = buffer.getWritePointer(1);
            outBuffer[sample] = signal;
        }
